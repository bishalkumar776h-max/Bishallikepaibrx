...
#FULLY MODIFIED BY TG @SILENT_CODEX DON'T REMOVE CREDIT IF YOU REMOVE CREDIT YOU ARE GAY
...
def handle_requests():
    ...
    if not uid or not server_name:
        err_response = {"error": "UID and server are required parameters"}
        return Response(json.dumps(err_response, indent=2), mimetype='application/json'), 400

    try:
        tokens = load_tokens(server_name)
        if not tokens: raise Exception("Failed to load tokens from storage configuration.")
        token = tokens[0]['token']
        encrypted_uid = enc(uid)
        if encrypted_uid is None: raise Exception("Encryption process of target UID failed.")

        before = make_request(encrypted_uid, server_name, token)
        if before is None: raise Exception("Failed to retrieve initial player data information.")

        data_before = json.loads(MessageToJson(before))
        before_like = data_before.get('AccountInfo', {}).get('Likes', 0)
        try: before_like = int(before_like)
        except Exception: before_like = 0
        ...
        if after is None: raise Exception("Failed to retrieve player data info after processing likes.")
